...
const Register: NextPageWithLayout = () => {
  ...
  const registerFunc: SubmitHandler<FormInput> = async (data) => {
    setRegistering(true);
    setError(null);
    ...
    try {
      axios
        .post("/api/id", { username: data.username })
        .then(() => {
          setRegistering(false);
          return setError("Username already in use");
        })
        .catch(async (error) => {
          if (error && error.response.data.id !== null) {
            setRegistering(false);
            return setError("Username already in use");
          }

          const { data: registerData } = await axios.post("/api/create", {
            name: data.username,
            password: data.password,
          });
          if (registerData.code !== 200) {
            setRegistering(false);
            return setError("Unexpected error, try again");
          }
  ...
  return (
    <div className="!font-inter">
      ...
      <div className="h-screen flex">
        <motion.div
          className="flex md:flex-none relative bg-primary-white dark:bg-primary rounded-none md:rounded-xl w-full min-h-screen md:w-fit md:h-fit md:min-h-fit px-16 md:px-[92px] py-[86px] m-auto z-50 scale-95"
